...
class BaseDescriptor(object):
    ...
    def init_cls(self, cls):
        ...
        ----------
        cls : class
            A class which has this descriptor in its mro
        """
        pass
...
class DataStruct(six.with_metaclass(MetaStruct, HasDescriptors)):
    ...
    @classmethod
    def del_fields(cls, *names):
        for n in names:
            if isinstance(getattr(cls, n), DataField):
                delattr(cls, n)
            else:
                m = "The name '%s' is not a field of '%s' instances"
                raise FieldError(m % (n, cls.__name__))
